    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in islice(train_loader, batch_per_epoch):
                ...
                pred,add_losses = parallel_model(train_data, batch)
...
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    ...
    for batch in test_loader:
        ...
        t_pred,add_losses = model(test_data, t_batch)
        h_pred,add_losses = model(test_data, h_batch)

        if filtered_data is None:
            t_mask, h_mask = tasks.strict_negative_mask(test_data, batch)
        else:
            t_mask, h_mask = tasks.strict_negative_mask(test_data, batch)  # DB15K,YAGO15K
...
if __name__ == "__main__":
    ...
    train_data, valid_data, test_data = dataset[0], dataset[2], dataset[2]
